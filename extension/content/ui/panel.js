  ...
  try {
    ...
    const response = await new Promise((resolve) => {
      chrome.runtime.sendMessage({
        type: 'UPLOAD_MEDIA', entityId: String(entityId), mediaType,
    ...
    if (response.ok) { showToast(`✅ ${label} загружено`, 'success'); return true; }
    else throw new Error(response.error || 'Ошибка');
  } catch (err) {
...
async function showMediaGallery(entityId, title) {
  ...
  modal.querySelector('#galleryContent').innerHTML = '<div style="text-align:center;padding:20px;color:#666;">⏳ Загружаем список...</div>';
  modal.style.display = 'flex';
  const response = await new Promise(resolve => {
    chrome.runtime.sendMessage({ type: 'GET_MEDIA_LIST', entityId: _galleryEntityId }, resolve);
  });
  if (!response?.ok) {
    modal.querySelector('#galleryContent').innerHTML = `<div style="color:#dc3545;padding:12px;">Ошибка: ${response?.error || 'неизвестно'}</div>`;
    return;
  }
...
async function renderGallery() {
  ...
  content.innerHTML = `
    <div style="margin-bottom:16px;">
      <div style="font-size:13px;font-weight:600;color:#1e3a5f;margin-bottom:8px;">📷 Фото (${photos.length})</div>
      <div id="galleryPhotos" style="display:flex;flex-wrap:wrap;gap:8px;min-height:40px;">
        ${photos.length === 0 ? '<span style="color:#aaa;font-size:12px;">нет фото</span>'
          : photos.map(m => `
            <div data-media-id="${m.id}" data-slide-idx="${photos.indexOf(m)}" style="position:relative;width:100px;height:100px;
              ...
              <img data-filename="${m.file_name}" src="" style="max-width:100%;max-height:100%;object-fit:cover;" title="${m.file_name}" />
              <span style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.5);color:white;font-size:9px;padding:2px 4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${m.file_name}</span>
              <button class="gallery-del-btn" data-id="${m.id}" style="position:absolute;top:2px;right:2px;background:rgba(220,53,69,0.85);color:white;border:none;border-radius:3px;cursor:pointer;font-size:10px;padding:1px 5px;line-height:1.4;">✕</button>
            </div>`).join('')}
      </div>
    </div>
    <div>
      <div style="font-size:13px;font-weight:600;color:#1e3a5f;margin-bottom:8px;">🎥 Видео (${videos.length})</div>
      <div id="galleryVideos" style="display:flex;flex-direction:column;gap:4px;min-height:20px;">
        ${videos.length === 0 ? '<span style="color:#aaa;font-size:12px;">нет видео</span>'
          : videos.map(m => {
              const kb = m.file_size ? ' (' + (m.file_size/1024).toFixed(0) + ' КБ)' : '';
              return `<div style="display:flex;align-items:center;gap:8px;padding:5px 8px;background:#f8f9fa;border:1px solid #dee2e6;border-radius:4px;">
                <span>🎥</span>
                <span class="gallery-video-play" data-slide-idx="${_galleryMediaList.indexOf(m)}"
                  style="flex:1;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;color:#1a5276;text-decoration:underline dotted;">${m.file_name}${kb}</span>
                <button class="gallery-del-btn" data-id="${m.id}" style="background:#dc3545;color:white;border:none;border-radius:3px;cursor:pointer;font-size:10px;padding:2px 7px;">✕</button>
              </div>`;
            }).join('')}
      </div>
    </div>`;
  content.querySelectorAll('img[data-filename]').forEach(async (img) => {
    ...
    const res = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_MEDIA_FILE', fileName, mimeType }, resolve));
  ...
  content.querySelectorAll('[data-slide-idx]').forEach(el => {
    el.addEventListener('click', async (e) => {
      ...
      await showSlideshow(_galleryMediaList, parseInt(el.getAttribute('data-slide-idx')), _galleryEntityId);
    });
  });
  ...
  content.querySelectorAll('.gallery-del-btn').forEach(btn => {
    btn.addEventListener('click', withLock(btn, async () => {
      ...
      const res = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'DELETE_MEDIA', id }, resolve));
      if (res?.ok) {
        _galleryMediaList = _galleryMediaList.filter(m => m.id !== id);
        await renderGallery();
        _syncMediaFlags(_galleryEntityId, _galleryMediaList);
        showToast('Файл удалён', 'success');
      } else showToast('Ошибка удаления: ' + (res?.error || ''), 'error');
    }, { label: '...' }));
  });
}
...
function _syncMediaFlags(entityId, mediaList) {
  ...
  wsSend({ type: 'UPDATE_TARGET_LOCAL', entity_id: String(entityId), has_photo: hasPhoto ? 1 : 0, has_video: hasVideo ? 1 : 0 });
}
...
async function _galleryUploadFiles(files, mediaType) {
  ...
  if (uploaded > 0) {
    const res = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_MEDIA_LIST', entityId: _galleryEntityId }, resolve));
    if (res?.ok) { _galleryMediaList = res.media || []; await renderGallery(); _syncMediaFlags(_galleryEntityId, _galleryMediaList); }
  }
}
...
// СЛАЙДЕР
// ══════════════════════════════════════════════════════════════════════════════
let _slideMedia = [], _slideIndex = 0, _slideEntityId = null;
let _slideDesc = '', _slideNotes = '', _slideBlobUrl = null;
...
async function showSlideshow(mediaList, startIndex, entityId) {
  ...
  if (!modal) return;
  modal.style.display = 'flex';
  const infoRes = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_TARGET_INFO', entityId: _slideEntityId }, resolve));
...
function _renderVideoPlaceholder(contentEl, item, mimeType) {
  ...
  contentEl.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;color:white;padding:30px;text-align:center;width:100%;box-sizing:border-box;">
      ...
      <button id="videoPlayBtn" style="padding:12px 32px;background:#2c7da0;color:white;border:none;border-radius:10px;cursor:pointer;font-size:17px;font-weight:700;">▶ Воспроизвести</button>
      <div id="videoProgressWrap" style="display:none;width:100%;max-width:360px;margin-top:20px;">
        <div style="background:rgba(255,255,255,0.15);border-radius:8px;height:10px;overflow:hidden;margin-bottom:8px;">
          <div id="videoProgressBar" style="height:100%;background:#2c7da0;width:0%;border-radius:8px;transition:width 0.3s ease;"></div>
        </div>
  ...
  contentEl.querySelector('#videoPlayBtn').addEventListener('click', () => {
    ...
    port.onMessage.addListener((msg) => {
      if (msg.type === 'PROGRESS') {
        ...
        if (bar) bar.style.width = Math.max(msg.progress >= 0 ? msg.progress : 0, 5) + '%';
        ...
        source.src = _slideBlobUrl; source.type = msg.mimeType;
        video.appendChild(source); contentEl.appendChild(video);
...
async function _renderSlide() {
  ...
  prevBtn.style.opacity = _slideIndex === 0 ? '0.25' : '1';
  prevBtn.style.cursor  = _slideIndex === 0 ? 'default' : 'pointer';
  nextBtn.style.opacity = _slideIndex === total-1 ? '0.25' : '1';
  nextBtn.style.cursor  = _slideIndex === total-1 ? 'default' : 'pointer';
  const dotsEl = modal.querySelector('#slideDots');
  dotsEl.innerHTML = _slideMedia.map((m, i) => `
    <span data-dot="${i}" style="display:inline-block;width:${i===_slideIndex?'12px':'9px'};height:${i===_slideIndex?'12px':'9px'};
      border-radius:50%;cursor:pointer;margin:0 4px;transition:0.2s ease;
      background:${i===_slideIndex?'#2c7da0':'#ccc'};
      border:${i===_slideIndex?'2px solid #1a5276':'2px solid transparent'};"></span>`).join('');
  dotsEl.querySelectorAll('[data-dot]').forEach(dot => {
    dot.addEventListener('click', async () => { _slideIndex = parseInt(dot.getAttribute('data-dot')); await _renderSlide(); });
  });
  ...
  const mimeMap = { jpg:'image/jpeg',jpeg:'image/jpeg',png:'image/png',webp:'image/webp',gif:'image/gif',bmp:'image/bmp',mp4:'video/mp4',mov:'video/quicktime',avi:'video/x-msvideo',webm:'video/webm',mkv:'video/x-matroska' };
  ...
  if (item.media_type === 'video') {
    ...
    if (descInput) descInput.value = _slideNotes || _slideDesc || '';
    return;
  }
  const res = await new Promise(resolve => chrome.runtime.sendMessage({ type: 'GET_MEDIA_FILE', fileName: item.file_name, mimeType }, resolve));
  if (!res?.ok) { contentEl.innerHTML = `<div style="color:#dc3545;text-align:center;padding:20px;">❌ Ошибка загрузки</div>`; return; }
  if (item.media_type === 'photo') {
    ...
    contentEl.innerHTML = ''; contentEl.appendChild(img);
  }
...
document.addEventListener('keydown', async (e) => {
  ...
  if (e.key === 'Escape') { modal.style.display = 'none'; if (_slideBlobUrl) { URL.revokeObjectURL(_slideBlobUrl); _slideBlobUrl = null; } }
});

// ══════════════════════════════════════════════════════════════════════════════
// ДАШБОРД ПОДРАЗДЕЛЕНИЙ
// ══════════════════════════════════════════════════════════════════════════════
...
async function fetchUnitTodayCount(folderId) {
  try {
    const entities = await apiFetchTargetsInFolder(folderId, null);
    const today = getMoscowDateStr(); // YYYY-MM-DD
...
async function renderUnitsDashboard() {
  ...
  dash.innerHTML = `
    <div style="font-size:13px;color:#888;text-align:center;padding:20px;">
      ⏳ Загружаем данные подразделений...
    </div>`;

  if (typeof UNIT_FOLDERS === 'undefined') {
    dash.innerHTML = '<div style="color:#dc3545;padding:20px;text-align:center;">UNIT_FOLDERS не определён</div>';
    return;
  }
  ...
  dash.innerHTML = entries.map(([key, unit], i) => {
    ...
    return `
      <div data-unit-key="${key}" data-folder-id="${unit.folderId}" data-unit-name="${unit.name}"
...
// Показать дашборд (скрыть таблицу)
...
function createPopup() {
  ...
  <div style="position:fixed;top:25px;right:20px;width:70%;max-height:calc(100vh - 40px);
    background:white;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.25);z-index:10;
    ...
    <style>
      ...
      #unitsDashboard { display:flex;flex-direction:column;gap:12px;overflow-y:auto;padding:16px;background:#f5f7fa; }
    </style>
    ...
    <div style="padding:5px 14px;background:#1e3a5f;color:white;display:flex;flex-direction:column;flex-shrink:0;">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          ...
          <button id="backToDashboardBtn" style="display:none;background:#6c757d;color:white;border:none;padding:6px 10px;border-radius:6px;cursor:pointer;font-size:13px;">◀ Подразделения</button>
    ...
    <div id="currentUnitLabel" style="display:none;padding:6px 14px;background:#0d1f33;color:#7aa3c8;font-size:12px;flex-shrink:0;"></div>
    ...
    <div id="dates-panel" style="background:#162d4a;padding:6px 14px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;flex-shrink:0;border-bottom:1px solid #0d1f33;min-height:36px;">
      ...
      <div style="margin-left:auto;display:flex;gap:6px;align-items:center;">
        <button id="publishPlanBtn" style="display:none;background:#28a745;border:none;color:white;border-radius:4px;padding:3px 10px;cursor:pointer;font-size:11px;">📤 Опубликовать план</button>
        <button id="refreshDatesBtn" style="background:none;border:1px solid #4a7a9b;color:#7aa3c8;border-radius:4px;padding:2px 8px;cursor:pointer;font-size:11px;">🔄</button>
      </div>
    </div>

    <!-- Дашборд подразделений -->
    <div id="unitsDashboard" style="flex:1;">
      <div style="text-align:center;color:#888;padding:20px;font-size:13px;">⏳ Загружаем...</div>
    </div>

    <!-- Таблица целей -->
    <div class="table-wrapper" style="display:none;flex:1;overflow-y:auto;padding:12px;background:#f5f7fa;color:black;">
    ...
    <div id="addTargetModal" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);display:none;justify-content:center;align-items:center;z-index:10000;">
      <div style="background:white;width:90%;max-width:520px;border-radius:10px;padding:20px;display:flex;flex-direction:column;gap:10px;max-height:90vh;overflow-y:auto;">
        ...
        <select id="targetType"><option value="" disabled selected>— Выберите категорию —</option></select>
        <label>Подразделение:</label>
        <select id="targetUnit"><option value="" disabled selected>— Выберите подразделение —</option></select>
      ...
      <div style="background:white;width:90%;max-width:640px;border-radius:12px;padding:20px;max-height:88vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.3);">
        ...
        <div style="display:flex;gap:8px;margin-top:12px;padding-top:12px;border-top:1px solid #eee;flex-shrink:0;">
          <label style="padding:7px 14px;background:#28a745;color:white;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">
            + Фото <input id="galleryPhotoInput" type="file" accept="image/*" multiple style="display:none;" />
          </label>
          <label style="padding:7px 14px;background:#17a2b8;color:white;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">
            + Видео <input id="galleryVideoInput" type="file" accept="video/*" multiple style="display:none;" />
          </label>
    ...
    <!-- ═══ Слайдер ═══ -->
    <div id="mediaSlideshowModal" style="position:fixed;inset:0;background:rgba(0,0,0,0.92);display:none;flex-direction:column;align-items:center;justify-content:center;z-index:10004;">
      <div style="width:100%;max-width:900px;display:flex;justify-content:space-between;align-items:center;padding:8px 16px;flex-shrink:0;">
        <span id="slideCounter" style="color:#aaa;font-size:13px;"></span>
        <button id="slideshowCloseBtn" style="background:none;border:none;color:white;font-size:32px;cursor:pointer;line-height:1;padding:0 8px;">&times;</button>
      </div>
      <div style="display:flex;align-items:center;justify-content:center;flex:1;width:100%;max-width:900px;gap:8px;min-height:0;padding:0 8px;">
        <button id="slidePrev" style="flex-shrink:0;background:rgba(255,255,255,0.1);border:none;color:white;font-size:32px;width:48px;height:80px;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;">&#8249;</button>
        <div id="slideContent" style="flex:1;min-width:0;max-height:60vh;display:flex;align-items:center;justify-content:center;overflow:hidden;"></div>
        <button id="slideNext" style="flex-shrink:0;background:rgba(255,255,255,0.1);border:none;color:white;font-size:32px;width:48px;height:80px;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;">&#8250;</button>
      </div>
        ...
        <div style="background:rgba(255,255,255,0.07);border-radius:8px;padding:10px 12px;">
          ...
          <div style="display:flex;gap:8px;align-items:flex-start;">
            <textarea id="slideDescInput" rows="2" placeholder="Введите описание объекта..."
              style="flex:1;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:6px;color:white;padding:6px 8px;font-size:13px;resize:none;font-family:system-ui,sans-serif;"></textarea>
            <button id="slideDescSaveBtn" style="padding:6px 14px;background:#2c7da0;color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;white-space:nowrap;align-self:flex-end;">💾 Сохранить</button>
          </div>
        </div>
      </div>
    </div>
  ...
  if (typeSelectEl && typeof buildTypeSelect === 'function') buildTypeSelect(typeSelectEl, '');
  ...
  // ── Кнопка "назад" ──────────────────────────────────────────────────────────
  popupElement.querySelector('#backToDashboardBtn').addEventListener('click', async () => {
    _showDashboard();
    await renderUnitsDashboard();
  });
  ...
  popupElement.querySelector('#targetPhoto').addEventListener('change', function() {
    ...
    if (this.files?.length) { nameEl.textContent = Array.from(this.files).map(f => f.name).join(', '); preview.style.display = 'block'; }
  ...
  submitBtn.addEventListener('click', withLock(submitBtn, async () => {
    try {
      ...
      const typeName       = TARGET_TYPE_MAP[typeCode] || typeCode;
      const characteristic = typeCode;
      ...
      if (newEntityId) {
        ...
        for (const file of Array.from(photoFiles)) { const ok = await uploadMediaFile(newEntityId, file, 'photo'); if (ok) hasPhoto = 1; }
        for (const file of Array.from(videoFiles)) { const ok = await uploadMediaFile(newEntityId, file, 'video'); if (ok) hasVideo = 1; }
    ...
    } catch (e) { console.error('[addTarget]', e); showToast('❌ Ошибка: ' + e.message, 'error'); }
  }, { label: '⏳ Добавляем...' }));

  // ── Галерея ────────────────────────────────────────────────────────────────
  popupElement.querySelector('#galleryCloseBtn').addEventListener('click', () => { document.querySelector('#mediaGalleryModal').style.display = 'none'; });
  popupElement.querySelector('#galleryPhotoInput').addEventListener('change', async function() { if (this.files?.length) await _galleryUploadFiles(this.files, 'photo'); this.value = ''; });
  popupElement.querySelector('#galleryVideoInput').addEventListener('change', async function() { if (this.files?.length) await _galleryUploadFiles(this.files, 'video'); this.value = ''; });

  popupElement.querySelector('#closePopupBtn').addEventListener('click', () => { popupElement.style.display = 'none'; });
  ...
  popupElement.querySelector('#showPlanningBtn').addEventListener('click', () => {
    ...
    document.querySelector('#unitsDashboard').style.display = 'none';
  ...
  popupElement.querySelector('#cancelNewTask').addEventListener('click', () => { newTaskModal.style.display = 'none'; });
  ...
  todayBtn.addEventListener('click', withLock(todayBtn, async () => { await loadByDateFromPanel(getMoscowDateStr()); }, { label: '⏳' }));
  ...
  popupElement.querySelector('#slidePrev').addEventListener('click', async () => { if (_slideIndex > 0) { _slideIndex--; await _renderSlide(); } });
  popupElement.querySelector('#slideNext').addEventListener('click', async () => { if (_slideIndex < _slideMedia.length-1) { _slideIndex++; await _renderSlide(); } });
  ...
  ssModal.addEventListener('click', (e) => {
    if (e.target === ssModal) { ssModal.style.display = 'none'; if (_slideBlobUrl) { URL.revokeObjectURL(_slideBlobUrl); _slideBlobUrl = null; } }
  });
...
function _resetAddTargetModal() {
  ...
  p.querySelector('#targetUnit').selectedIndex = 0;
...
function closeBtn() {
  ...
  if (btn && !btn.hasListener) {
    btn.addEventListener('click', () => { const popup = document.querySelector('#extension-popup'); if (popup) popup.style.display = 'none'; });
...
function findAndAddButton() {
  ...
  btn.onclick = async (e) => {
    ...
    if (isOpening) {
      renderDatePanel().then(() => { initBadgesFromCache(); loadAllDatesBadgesInBackground(); restoreDraftDateBtns(); });
      updateRoleTag();
      if (typeof _renderOnlineIndicator === 'function') _renderOnlineIndicator();
      // Показываем дашборд при открытии
      _showDashboard();
      await renderUnitsDashboard();
    }
  };
